 ...
 */
public class PrefCostModule extends AbstractDvrpModeQSimModule {
    
    private final boolean usePreferenceWeights;
    
    public PrefCostModule(String mode, boolean usePreferenceWeights) {
        super(mode);
        this.usePreferenceWeights = usePreferenceWeights;
        System.out.println("PrefCostModule: Created for mode '" + mode + "' with preference weights: " + usePreferenceWeights);
    }
    
    @Override
    protected void configureQSim() {
        System.out.println("PrefCostModule: Configuring QSim-level bindings for mode: " + getMode());
        System.out.println("PrefCostModule: Using preference weights: " + usePreferenceWeights);
        ...
        System.out.println("PrefCostModule: Bound PrefAwareInsertionCostCalculator as InsertionCostCalculator for mode: " + getMode());
    }
    ...
    public UserPreferenceStore provideUserPreferenceStore() {
        System.out.println("PrefCostModule: Loading user preference data...");
        UserPreferenceStore store = PreferenceDataLoader.loadAllPreferenceData();
        System.out.println("PrefCostModule: Loaded preferences for " + store.getTotalUsers() + " users");
        return store;
    }
}